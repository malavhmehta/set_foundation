...
const StyledNav = styled.nav.attrs(({
  className: "container navbar navbar-expand-lg navbar-light bg-light fixed-top"
}))`
...
export class Nav extends Component {
  ...
  handleScroll = () => {
    ...
    if (pos - this.state.prev <= - this.state.delta || pos === 0 || this.state.toggled) {
      this.setState({
        prev: pos,
        show: true
      });
            ...
            <ul className="navbar-nav ml-auto">
              {this.props.navbar.links.map((link) => (
                  <li className="nav-item" key={link.href}>
                    <NavLink active={(link.href === pathname || hover === link.href) ? "active" : ""} to={link.href} onClick={this.scrollTop} onMouseEnter={() => this.setHover(link.href)} onMouseLeave={() => this.setHover("")}>
                      {link.caption}
                    </NavLink>

                    {link.anchors && (
                        <Dropdown show={hover === link.href} onMouseEnter={() => show && this.setHover(link.href)} onMouseLeave={() => this.setHover("")}>
                          {link.anchors.map((anchor) => (
                              <DropdownLink key={anchor.href} to={link.href + anchor.href} disabled={!show}>
                                {anchor.caption}
                              </DropdownLink>
                          ))}
                        </Dropdown>
          ...
          <MobileNav active={toggled} onClick={this.toggleNav}>
            ...
            {this.props.navbar.links.map((link) => (
                <div className="nav-item" key={link.href}>
                  <MobileNavLink active={(link.href === pathname || hover === link.href) ? "active" : ""} to={link.href} onClick={this.scrollTop}>
                    {link.caption}
                  </MobileNavLink>
                  ...
                  {link.anchors && (
                      <MobileDropdown show={hover === link.href}>
                        {link.anchors.map((anchor) => (
                            <MobileDropdownLink key={anchor.href} to={link.href + anchor.href} disabled={!show}>
                              {anchor.caption}
                            </MobileDropdownLink>
                        ))}
                      </MobileDropdown>
